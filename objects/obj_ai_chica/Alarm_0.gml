...
if(obj_ai_chica.ai_level >= _move) {
	...
	if(obj_ai_chica.current_camera == "RightWindow")  {
		_forward = !obj_core.gameplay.right_door;
		if(_forward) {
			obj_ai_chica.current_camera = "Attack";
			obj_core.gameplay.right_light = false;
	...
	if( scr_camera_is_camera_up() && obj_core.gameplay.camera.current_camera == obj_ai_chica.current_camera) obj_camera_current_spr.disable_camera();
