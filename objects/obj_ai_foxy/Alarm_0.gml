...
if(obj_ai_foxy.ai_level >= _move && obj_ai_foxy.camera_down_time == 0.0) {
	...
	if(obj_core.gameplay.camera.camera_up && obj_ai_foxy.current_camera != "2A") return;
	...
	if(obj_ai_foxy.current_camera == "2A" && !obj_ai_foxy.is_running)  {
		if(obj_core.gameplay.left_door) obj_ai_foxy.current_camera = "AttackFail";
		else obj_ai_foxy.current_camera = "AttackSuccess";
		obj_core.gameplay.current_foxy_cam = current_camera;
