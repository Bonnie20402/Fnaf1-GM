...
function PowerModel() constructor {
    ...
    /// @description  Runs a power cycle.
    /// @param {Struct.Gameplay} _gameplay : An reference of a gameplay struct.
    function run_power_cycle(_gameplay) {
        if(power_points == 0 && !power_out)  { on_power_out(); return; }

        power_points-=get_current_usage(_gameplay);
    ...
    function get_current_usage(_gameplay) {
        return 1 + (_gameplay.office.left_door + _gameplay.office.right_door + 
